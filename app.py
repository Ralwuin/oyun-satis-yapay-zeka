...
# 3. HAFIZA KATMANI BAŞLATMA (Session State)
...
# 6. TAHMİN TETİKLEME VE HAFIZAYA ALMA
if tahmin_butonu:
...
if st.session_state.tahmin_edildi:
    ...
    for i, platform in enumerate(platformlar):
        with sekmeler[i]:
            ...
            kutu1.metric("🇺🇸 Kuzey Amerika Potansiyeli", f"{na:.2f} Milyon")
            kutu2.metric("🇪🇺 Avrupa Pazar Potansiyeli", f"{eu:.2f} Milyon")
            kutu3.metric("🇯🇵 Japonya Pazar Potansiyeli", f"{jp:.2f} Milyon")
            kutu4.metric("🌍 Global Öngörülen Toplam", f"{toplam:.2f} Milyon")
